...
import { ArrowLeft, Globe, CheckCircle, XCircle, Search, Monitor, Loader2, Network, ArrowUp, ArrowDown } from 'lucide-react';
...
const TargetAssets = () => {
  ...
  useEffect(() => {
    setPage(1);
  }, [filterLive, filterHttpx, filterDnsOnly]);

  // 👇 هندلرهای هوشمند برای دکمه‌ها (Mutual Exclusion)
  const toggleHttpx = () => {
      if (!filterHttpx) {
          setFilterDnsOnly(undefined); // خاموش کردن DNS Only اگر Has Web روشن شد
          setFilterHttpx(true);
      } else {
  ...
  const toggleDnsOnly = () => {
      if (!filterDnsOnly) {
          setFilterHttpx(undefined); // خاموش کردن Has Web اگر DNS Only روشن شد
  ...
  const assetsQuery = useQuery({
    ...
    queryFn: () => getTargetAssets(
      ...
      { 
        ...
        dns_only: filterDnsOnly // 👈 ارسال به API
      },
      sortBy, sortOrder
    ),
    enabled: !!targetId,
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className={clsx(
          ...
          assetsQuery.isFetching ? "opacity-60 pointer-events-none" : "opacity-100"
      )}>
        ...
        <div className="border-t border-gray-800 p-4 flex justify-between items-center bg-gray-900 mt-auto flex-shrink-0">
           ...
           <div className="flex gap-2">
             <button disabled={page === 1 || assetsQuery.isFetching} onClick={() => setPage(p => Math.max(1, p - 1))} className="px-3 py-1 rounded border border-gray-700 text-gray-300 text-sm disabled:opacity-50 hover:bg-gray-800 disabled:cursor-not-allowed">Previous</button>
             <button disabled={!assetsQuery.data?.data || assetsQuery.data.data.length < 50 || assetsQuery.isFetching} onClick={() => setPage(p => p + 1)} className="px-3 py-1 rounded border border-gray-700 text-gray-300 text-sm disabled:opacity-50 hover:bg-gray-800 disabled:cursor-not-allowed">Next</button>
           </div>
        </div>
      </div>
    </div>
