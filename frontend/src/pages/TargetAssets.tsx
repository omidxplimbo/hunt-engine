...
import { ArrowLeft, Globe, CheckCircle, XCircle, Search, Monitor, Loader2 } from 'lucide-react';
...
const TargetAssets = () => {
  ...
  const assetsQuery = useQuery({
    // کلید یونیک شامل تمام پارامترهاست
    ...
    enabled: !!targetId,
    // 👇 حذف placeholderData یا استفاده از keepPreviousData برای UX بهتر
    // اگر می‌خواهی صفحه سفید نشود ولی دیتا عوض شود، این خط را بردار و از isFetching استفاده کن
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* Table Container */}
      <div className={clsx(
          "bg-gray-900 rounded-lg border border-gray-800 flex flex-col overflow-hidden transition-opacity duration-200",
          assetsQuery.isFetching ? "opacity-60 pointer-events-none" : "opacity-100" // 👈 افکت لودینگ واضح
      )}>
            ...
            <table className="w-full text-left min-w-[1400px]"> 
              ...
              <tbody className="divide-y divide-gray-800">
                {assetsQuery.data?.data.map((asset) => {
                  ...
                  } catch { if (asset.dnsx_ip) dnsxIps = [asset.dnsx_ip]; }

                  let httpxIps: string[] = [];
                  try {
                     const parsed = JSON.parse(asset.host_ip || "[]");
                     httpxIps = Array.isArray(parsed) ? parsed : [parsed];
                  } catch { if (asset.host_ip) httpxIps = [asset.host_ip]; }
                  ...
                  <tr key={asset.id} className="hover:bg-gray-800/30 transition-colors">
                    <td className="px-6 py-4 align-top">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          <a href={`http://${asset.value}`} target="_blank" rel="noreferrer" className="font-medium text-gray-200 hover:text-blue-400 transition-colors break-all">
                            {asset.value}
                          </a>
                        </div>
                      </div>
                    ...
                    <td className="px-6 py-4 align-top">
                      <div className="flex flex-col gap-1.5">
                        {dnsxIps.length > 0 && dnsxIps[0] !== "" ? (
                          dnsxIps.map((ip, idx) => (
                            <span key={idx} className="text-xs font-mono text-cyan-400 bg-cyan-950/30 px-2 py-0.5 rounded border border-cyan-900 w-fit">{ip}</span>
                          ))
                    ...
                    <td className="px-6 py-4 align-top">
                      <div className="flex flex-col gap-1.5">
                        {httpxIps.length > 0 && httpxIps[0] !== "" ? (
                          httpxIps.map((ip, idx) => (
                            <span key={idx} className="text-xs font-mono text-gray-400 bg-gray-950 px-2 py-0.5 rounded border border-gray-800 w-fit">{ip}</span>
                          ))
                        ) : <span className="text-gray-600 text-xs">-</span>}
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4 align-top">
                      ...
                      ) : <span className="px-2 py-1 rounded text-xs font-bold bg-gray-800 text-gray-500 border border-gray-700">DEAD</span>}
                    </td>

                    <td className="px-6 py-4 align-top">
                      <span className="text-sm text-gray-300 block line-clamp-2" title={asset.title}>{asset.title || '-'}</span>
                    </td>
                    ...
                    <td className="px-6 py-4 align-top">
                      <div className="flex flex-wrap gap-1 max-h-[100px] overflow-y-auto">
                        ...
                        {techs.map((tech, i) => (
                          <span key={i} className="px-1.5 py-0.5 rounded bg-purple-900/30 text-purple-300 text-[10px] border border-purple-800 whitespace-nowrap">{tech}</span>
                        ))}
                      </div>
                    </td>
        ...
        <div className="border-t border-gray-800 p-4 flex justify-between items-center bg-gray-900 mt-auto flex-shrink-0">
           ...
           <div className="flex gap-2">
             ...
             <button 
               disabled={!assetsQuery.data?.data || assetsQuery.data.data.length < 50 || assetsQuery.isFetching}
