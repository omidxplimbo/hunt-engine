...
const TargetAssets = () => {
  ...
  const SortableHeader = ({ field, label, className }: { field: string, label: string, className?: string }) => (
    <th 
      className={clsx("px-6 py-3 cursor-pointer hover:text-hack-primary transition-colors select-none group border-b border-hack-border font-mono text-xs text-hack-dim uppercase tracking-wider", className)}
  ...
  const assetsQuery = useQuery({
    ...
    queryFn: () => getTargetAssets(targetId, page, 50, { is_live: filterLive, search: debouncedSearch, has_httpx: filterHttpx, dns_only: filterDnsOnly }, sortBy, sortOrder),
  ...
  return (
    <div className="flex flex-col h-full space-y-6">
      <div className="flex items-center gap-4 flex-shrink-0 border-b border-hack-border/50 pb-4">
        ...
        <div>
          <div className="flex items-center gap-3">
            <h1 className="hack-title text-xl">
              {targetQuery.isLoading ? 'LOADING...' : targetQuery.data?.name}
            </h1>
            <span className="hack-badge border-hack-primary/30 text-hack-primary bg-hack-primary/10 font-mono lowercase">
      ...
      <div className="flex gap-2">
          <button onClick={() => setActiveTab('assets')} className={clsx("hack-btn", activeTab === 'assets' ? "bg-hack-primary text-black" : "bg-transparent text-hack-dim border-hack-dim/30")}>
              <Database size={14} /> Assets
          </button>
          <button onClick={() => setActiveTab('urls')} className={clsx("hack-btn", activeTab === 'urls' ? "bg-hack-primary text-black" : "bg-transparent text-hack-dim border-hack-dim/30")}>
              <Link2 size={14} /> Intel
          </button>
      </div>

      <div className="hack-box p-2 flex flex-wrap items-center gap-4 justify-between flex-shrink-0">
        <div className="relative flex-1 min-w-[200px] max-w-md flex items-center bg-black/40 border border-hack-border px-3">
          <Search className="text-hack-dim" size={14} />
          <input 
            type="text" 
            placeholder={activeTab === 'assets' ? "QUERY ASSETS..." : "QUERY INTEL..."}
            className="w-full bg-transparent border-none text-hack-primary pl-3 py-2 focus:ring-0 text-sm font-mono placeholder-hack-dim/50"
            value={searchTerm}
        ...
        {activeTab === 'assets' && (
            <div className="flex items-center gap-2">
                <span className="text-[10px] uppercase text-hack-dim tracking-widest">Filters:</span>
                <div className="flex gap-1">
                    <button onClick={() => setFilterLive(undefined)} className={clsx("hack-btn-ghost border border-transparent", filterLive === undefined && "border-hack-dim/50 text-white")}>All</button>
                    <button onClick={() => setFilterLive(true)} className={clsx("hack-btn-ghost flex items-center gap-1", filterLive === true && "!text-hack-primary border border-hack-primary/50")}><CheckCircle size={12} /> Live</button>
                    <button onClick={() => setFilterLive(false)} className={clsx("hack-btn-ghost flex items-center gap-1", filterLive === false && "!text-hack-danger border border-hack-danger/50")}><XCircle size={12} /> Dead</button>
                </div>
                <div className="w-px bg-hack-border h-4 mx-2"></div>
                <button onClick={toggleHttpx} className={clsx("hack-btn-ghost flex items-center gap-1 border", filterHttpx ? "border-hack-primary text-hack-primary" : "border-hack-border")}><Monitor size={12} /> Web</button>
                <button onClick={toggleDnsOnly} className={clsx("hack-btn-ghost flex items-center gap-1 border", filterDnsOnly ? "border-hack-warning text-hack-warning" : "border-hack-border")}><Network size={12} /> DNS</button>
            </div>
        )}

        {activeTab === 'urls' && (
            <div className="flex items-center gap-2">
                  <button onClick={() => setFilterJsOnly(!filterJsOnly)} className={clsx("hack-btn-ghost flex items-center gap-1 border", filterJsOnly ? "border-hack-warning text-hack-warning" : "border-hack-border")}>
      ...
      <div className="hack-box flex flex-col flex-1 overflow-hidden relative">
        {/* Corner Decor */}
        <div className="absolute top-0 right-0 p-1 border-b border-l border-hack-primary/20 bg-hack-primary/5 text-[8px] text-hack-primary font-mono">DATA_GRID_V1</div>
                ...
                {activeTab === 'assets' ? assetsQuery.data?.data.map((asset) => {
                    ...
                    <tr key={asset.id} className="hover:bg-hack-primary/5 transition-colors font-mono text-sm group">
                        ...
                        <div className="flex items-start gap-3">
                            <Globe size={14} className={asset.is_live ? "text-hack-primary mt-1" : "text-hack-dim mt-1"} />
                            <div><a href={`http://${asset.value}`} target="_blank" rel="noreferrer" className="text-gray-300 hover:text-hack-primary transition-colors break-all group-hover:underline underline-offset-4">{asset.value}</a></div>
                        </div>
                        </td>
                        ...
                        <td className="px-6 py-3 align-top"><span className="text-xs text-hack-text block line-clamp-2 opacity-80" title={asset.title}>{asset.title || '-'}</span></td>
                        <td className="px-6 py-3 align-top"><div className="flex flex-wrap gap-1 max-h-[60px] overflow-y-auto">{asset.web_server && <span className="px-1 py-0.5 bg-white/5 text-[9px] border border-white/10 text-hack-text whitespace-nowrap">{asset.web_server}</span>}{techs.map((tech, i) => <span key={i} className="px-1 py-0.5 bg-hack-primary/5 text-[9px] border border-hack-primary/20 text-hack-primary whitespace-nowrap">{tech}</span>)}</div></td>
                    ...
                    <tr key={url.id} className="hover:bg-hack-primary/5 transition-colors group font-mono text-sm">
                        <td className="px-6 py-3 align-top">
                            <div className="flex items-start gap-3">
                                ...
                                <a href={url.value} target="_blank" rel="noreferrer" className={clsx("transition-colors break-all block leading-relaxed hover:underline underline-offset-4", url.value.endsWith('.js') ? "text-hack-warning hover:text-white" : "text-hack-dim hover:text-hack-primary")}>{url.value}</a>
                            </div>
                        </td>
                        ...
                        <td className="px-6 py-3 align-top text-right text-xs text-hack-dim">{new Date(url.created_at).toLocaleString()}</td>
                    </tr>
                    ))}
            </tbody>
            </table>
        ...
        <div className="border-t border-hack-border p-2 flex justify-between items-center bg-black/60 text-xs font-mono">
