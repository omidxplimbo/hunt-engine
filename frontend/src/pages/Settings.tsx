  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="font-mono text-2xl uppercase tracking-wider text-hack-primary">SYSTEM CONFIGURATION</h1>
          <p className="text-hack-dim font-mono text-sm">Access Control & Parameters</p>
        </div>
        <button
          onClick={() => {
            setEditingUser(null);
      ...
      <div className="border border-hack-border bg-black/30 p-5">
        <h2 className="mb-4 font-mono text-lg uppercase tracking-wider text-hack-primary">User_Privileges_DB</h2>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left font-mono text-sm">
            ...
            <tbody>
              {isLoading ? (
                <tr>
                  <td colSpan={6} className="py-4 text-hack-dim">DECRYPTING USER DATA...</td>
                </tr>
              ) : (
                data?.data.map((user: any) => (
                  <tr key={user.id} className="border-b border-hack-border/50">
                    <td className="py-3 pr-4 text-white flex items-center gap-2">
                      <User size={16} className="text-hack-primary" /> {user.username}
                    </td>
                    <td className="py-3 pr-4 uppercase text-hack-primary">{user.role}</td>
                    <td className="py-3 pr-4">
                      <span className="inline-flex items-center gap-2 text-hack-dim">
                        <ListOrdered size={14} /> {user.role === 'admin' ? 'UNLIMITED' : user.max_concurrent_scans || 1}
                      </span>
                    </td>
                    <td className="py-3 pr-4">{user.is_active ? 'active' : 'deactive'}</td>
                    <td className="py-3 pr-4 text-hack-dim">{new Date(user.created_at).toLocaleDateString()}</td>
                    <td className="py-3 text-right">
                      <div className="inline-flex items-center gap-2">
                        ...
                        <button
                          onClick={() => {
                            if (confirm('>> EXECUTE DELETION PROTOCOL?')) deleteMutation.mutate(user.id);
                          }}
