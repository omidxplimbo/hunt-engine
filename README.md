...
- **PROBING (Optional)**: NUCLEI scan can generate **Findings** (controlled via env)
...
* `GET /api/targets/:id/findings` - List target findings (filters: `search`, `severity`, `status`, `type`)
* `PATCH /api/findings/:id` - Update finding triage fields (`status`, `notes`, `reported_to`)
...
## 🔎 Findings & Nuclei (Optional)

The platform can store **Findings** (triageable issues) generated by scanners such as `nuclei`.

### Enable nuclei-based findings
Set these env vars on the backend container:
- `ENABLE_NUCLEI=true`
- `NUCLEI_SEVERITIES=critical,high` (default if omitted)
- `NUCLEI_TAGS=` (optional, e.g. `takeover`)

When enabled, the worker runs a nuclei scan after PROBING and stores results into the `findings` table (deduplicated per target).
