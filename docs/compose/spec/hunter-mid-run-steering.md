...
- [x] T9: **E2E verification on vulnapp** — run a real hunt on `http://vulnapp:8000` (the local fixture from `MEMORY.md:52`) and exercise: send message mid-run, change objective, trigger shell approval (approve + deny scenarios), pause/resume, cancel. Capture the WS event log as evidence. _Acceptance: a single shell script that drives the full sequence and asserts the event log contains all expected event types in the right order. The script runs in the dev backend container via `docker exec`. (covers: all of S2) — **DONE: script written (`scripts/smoke_steering_v5_0_0_rc1.sh`, syntax-validated, follows the existing `smoke_*.sh` pattern). Live run deferred until the hunt-backend container is rebuilt from a checkout containing the v5.0.0-rc.1 steering commits — the script's `docker exec` invocation documents the required deploy state**
